...
class MainViewController: UITabBarController {
    ...
    @objc func playerProviderNowPlayingInfoDidChange(_ sender: Notification) {
        ...
        let image = UIImage(named: playerProvider.isPodcastPlaying() ? "PauseBlack" : "PlayBlack")
        playerView.playButton.setImage(image, for: .normal)
        playerView.thumbImageView.kf.setImage(with: URL(string: episode?.imageUrl ?? "")) { (result) in
            switch result {
            case.success:
                self.playerView.thumbImageView.contentMode = .scaleAspectFill
            case .failure:
                self.playerView.thumbImageView.contentMode = .center
                self.playerView.thumbImageView.image = UIImage(systemName: "photo")
            }
        }
    }
    ...
    func showPlayerView() {
        ...
        viewControllers?.forEach({ viewController in
            viewController.additionalSafeAreaInsets = UIEdgeInsets(top: 0, left: 0, bottom: 140, right: 0)
        })
    }
