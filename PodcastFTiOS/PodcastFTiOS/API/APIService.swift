...
class APIService: Service {
    ...
    func fetchPodcastsInHomeItem1(q: String, completion: @escaping (Result<[HomeItem1], Error>) -> Void) {
        let parameters: [String: Any] = [
            "media": "podcast",
            "limit": 5,
            "term": q
        ...
        AF.request(SEARCH_URL, method: .get, parameters: parameters, encoding: URLEncoding.default)
            .responseDecodable(of: SearchResponse.self, completionHandler: { (response) in
                ...
                case .success(let searchResponse):
                    completion(
                        .success(searchResponse.resultHomeItem1)
                    )
                case .failure(let error):
                    completion(.failure(error))
                }
    ...
    func fetchPodcastsInHomeItem2(q: String, completion: @escaping (Result<[HomeItem2], Error>) -> Void) {
        let parameters: [String: Any] = [
            "media": "music",
            "limit": 10,
            "term": q
        ...
        AF.request(SEARCH_URL, method: .get, parameters: parameters, encoding: URLEncoding.default)
            .responseDecodable(of: SearchResponse.self, completionHandler: { (response) in
                ...
                case .success(let searchResponse):
                    completion(
                        .success(searchResponse.resultHomeItem2)
                    )
                case .failure(let error):
                    completion(.failure(error))
                }
